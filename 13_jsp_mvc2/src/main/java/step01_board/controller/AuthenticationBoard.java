...
public class AuthenticationBoard extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/*
		 * 수정, 삭제 버튼을 눌렀을 때 인증화면으로 이동시키는 메서드
		 * */
		
		request.getParameter("menu"); //menu받음
		long boardId = Long.parseLong(request.getParameter("boardId")); //boardId 받음(?뒤에)
		BoardDTO boardDTO = BoardDAO.getInstance().getBoardDetail(boardId); //boardDTO가져올 수 있음
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * bAuthentication.jsp에서 submit이후를 처리하는 메서드
		 * */
		...
		//boardId, password를 전달하고 사용자 인증 결과에 따라 true, false반환
		boolean isAuthenticationUser = BoardDAO.getInstance().checkAuthenticationUser(boardDTO);//boardDAO에 checkAuthenticationUser필요
